...
export default function TableComponent({
  ...
  if (isLoading) {
    // ⬅️ mostrar skeleton SOLO cuando está cargando
    return <InspectionTableSkeleton />;
  }

  if (applications.length === 0) {
    // ⬅️ estado vacío
    return (
      <div className="border border-gray-300 rounded-[4px] p-8 text-center text-gray-500">
                ...
                <td className="p-0">
                  <div className="flex justify-center items-center gap-3 h-full min-h-[48px] px-3">
                    {actions.map(({ action, icon }, idx) => {
                      if (action === "play" && !["En curso", "Pendiente", "En Cola"].includes(item.status)) {
                        return null;
                      }
                      return (
