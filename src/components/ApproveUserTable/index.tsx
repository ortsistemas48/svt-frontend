...
export default function ApproveWorkshopTable({ workshops }: { workshops: Workshop[] }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 mt-3 sm:mt-4 md:mt-6 px-1 sm:px-2 md:px-4 lg:px-6">
      {/* Filtros y acciones */}
      <div className="flex flex-row gap-2 sm:gap-3 mt-4 sm:mt-6 w-full">
        ...
        <div className="flex gap-2 sm:gap-3 flex-shrink-0">
          <button className="hidden sm:flex bg-[#0040B8] hover:bg-[#0035A0] text-white px-3 md:px-4 py-2 sm:py-3 rounded-[4px] items-center justify-center gap-2 transition-colors duration-200 font-medium text-sm h-10 md:h-12">
            <SlidersHorizontal size={16} className="w-4 h-4" />
            <span>Filtrar</span>
          </button>

          <button
            className="hidden sm:flex bg-white border-2 border-[#0040B8] px-3 md:px-4 py-2 sm:py-3 rounded-[4px] items-center justify-center gap-2 hover:bg-[#0040B8] hover:text-white transition-colors duration-200 font-medium text-sm h-10 md:h-12"
            onClick={handleRefresh}
          >
            <RefreshCcw size={16} className="w-4 h-4 text-[#0040B8]" />
            <span className="text-[#0040B8]">Actualizar</span>
          </button>
        </div>
      </div>

      {/* Controles de paginación superiores */}
      <div className="mt-1 sm:mt-4 w-full flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
      ...
      {/* Tabla - vista desktop */}
      <div className="hidden xl:block border border-gray-300 mt-2 sm:mt-3 rounded-lg sm:rounded-[4px] overflow-hidden w-full">
        <table className="w-full text-sm">
          <thead className="bg-[#ffffff] text-[#00000080]">
            <tr>
              ...
              <th className="p-2 sm:p-3 text-center text-xs sm:text-sm font-medium">Localidad</th>
            ...
            {pageWorkshops.length === 0 ? (
              ...
              pageWorkshops.map((ws) => (
                <tr key={ws.id} className="border-t">
                  <td className="p-2 sm:p-3 text-center text-xs sm:text-sm font-medium">{ws.name}</td>
                  <td className="p-2 sm:p-3 text-center text-xs sm:text-sm">{ws.razon_social || ws.razonSocial || "-"}</td>
                  <td className="p-2 sm:p-3 text-center text-xs sm:text-sm">{ws.city}, {ws.province}</td>
                  <td className="p-2 sm:p-3 text-center text-xs sm:text-sm">{ws.cuit || "-"}</td>
                  <td className="p-0">
      ...
      <div className="xl:hidden mt-2 sm:mt-3 space-y-2 sm:space-y-3 w-full">
        {pageWorkshops.length === 0 ? (
          <div className="text-center py-8 sm:py-12 text-xs sm:text-sm text-gray-600 bg-white rounded-lg border border-gray-300 p-4">
            No hay talleres para aprobar.
          </div>
        ) : (
          pageWorkshops.map((ws) => (
            <div
              key={ws.id}
              className="bg-white border border-gray-300 rounded-lg p-3 sm:p-4 space-y-2 sm:space-y-3"
            >
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => openWsModal(ws)}
                  className="text-[#0040B8] hover:opacity-80 p-1.5 rounded hover:bg-blue-50 transition-colors flex-shrink-0 ml-2"
                  aria-label="Ver detalles"
                  title="Ver detalles"
                >
              ...
              <div className="grid grid-cols-1 gap-2 sm:gap-3 text-xs sm:text-sm">
                <div>
                  ...
                  <div className="text-gray-900 break-all">{ws.razon_social || ws.razonSocial || "-"}</div>
                </div>
                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                  <div>
                    <div className="text-gray-500 mb-0.5">Localidad</div>
                    <div className="text-gray-900">{ws.city}</div>
                  </div>
                  <div>
                    <div className="text-gray-500 mb-0.5">Provincia</div>
                    <div className="text-gray-900">{ws.province}</div>
                  </div>
                </div>
                <div>
      ...
      <div className="w-full flex items-center justify-between gap-2 sm:gap-3 mt-3 sm:mt-4">
        <button
          ...
          className={`px-2 py-1 rounded border text-xs ${currentPage <= 1 ? "text-gray-400 border-gray-200" : "text-[#0040B8] border-[#0040B8]"}`}
        >
          Anterior
        </button>
        ...
        <button
          ...
          className={`px-2 py-1 rounded border text-xs ${currentPage >= totalPages ? "text-gray-400 border-gray-200" : "text-[#0040B8] border-[#0040B8]"}`}
        >
          Siguiente
        </button>
      </div>
      ...
      {openModal && selectedWs && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-2 sm:p-4">
          ...
          <div className="relative z-[61] w-full max-w-4xl bg-white rounded-lg sm:rounded-2xl shadow-xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col">
            {/* Header */}
            <div className="flex items-center justify-between px-3 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 border-b bg-white">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="p-1.5 sm:p-2 rounded-full bg-[#F3F6FF]">
                  <Building2 size={16} className="sm:w-5 sm:h-5 text-[#0040B8]" />
                </div>
                <div>
                  <h3 className="text-sm sm:text-base md:text-lg font-semibold text-gray-900">Detalles del Taller</h3>
                  <p className="text-xs sm:text-sm text-gray-600 truncate max-w-[200px] sm:max-w-none">{selectedWs.name}</p>
                </div>
              </div>
              <button
                onClick={closeModal}
                className="p-1 sm:p-1.5 rounded-full border hover:bg-gray-50 transition-colors flex-shrink-0"
                disabled={!!actionBusy}
                aria-label="Cerrar"
              >
                <X size={16} className="sm:w-[18px] sm:h-[18px] text-gray-700" />
              </button>
            </div>

            {/* Content */}
            <div className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-6 space-y-4 sm:space-y-5 md:space-y-6 overflow-y-auto flex-1">
              {loadingDetail ? (
                ...
                <>
                  ...
                  <div>
                    <h4 className="text-xs sm:text-sm font-semibold text-gray-700 mb-3 sm:mb-4 flex items-center gap-2">
                      <Building2 size={14} className="sm:w-4 sm:h-4 text-[#0040B8]" />
                      Información General
                    </h4>
                    <div className="bg-gray-50 rounded-lg sm:rounded-[14px] p-3 sm:p-4 space-y-2 sm:space-y-3">
                      <DetailRow 
                        icon={<Factory size={16} className="sm:w-[18px] sm:h-[18px] text-gray-500" />}
                        label="Nombre del taller" 
                        value={wsDetail.name} 
                      />
                      <DetailRow 
                        icon={<Building2 size={16} className="sm:w-[18px] sm:h-[18px] text-gray-500" />}
                        label="Razón social" 
                  ...
                  <div>
                    <h4 className="text-xs sm:text-sm font-semibold text-gray-700 mb-3 sm:mb-4 flex items-center gap-2">
                      <MapPin size={14} className="sm:w-4 sm:h-4 text-[#0040B8]" />
                      Ubicación
                    </h4>
                    <div className="bg-gray-50 rounded-lg sm:rounded-[14px] p-3 sm:p-4 space-y-2 sm:space-y-3">
                      <DetailRow 
                        icon={<MapPin size={16} className="sm:w-[18px] sm:h-[18px] text-gray-500" />}
                        label="Provincia" 
                      ...
                      />
                      <DetailRow 
                        icon={<Home size={16} className="sm:w-[18px] sm:h-[18px] text-gray-500" />}
                        label="Dirección" 
                  ...
                  <div>
                    <h4 className="text-xs sm:text-sm font-semibold text-gray-700 mb-3 sm:mb-4 flex items-center gap-2">
                      <FileText size={14} className="sm:w-4 sm:h-4 text-[#0040B8]" />
                      Información Legal y Contacto
                    </h4>
                    <div className="bg-gray-50 rounded-lg sm:rounded-[14px] p-3 sm:p-4 space-y-2 sm:space-y-3">
                      <DetailRow 
                        icon={<FileText size={16} className="sm:w-[18px] sm:h-[18px] text-gray-500" />}
                        label="CUIT" 
                        value={wsDetail.cuit || "-"} 
                      />
                      <DetailRow 
                        icon={<Mail size={16} className="sm:w-[18px] sm:h-[18px] text-gray-500" />}
                        label="Email del taller" 
                        value={workshopOwnerEmail} 
                      />
                      <DetailRow 
                        icon={<Phone size={16} className="sm:w-[18px] sm:h-[18px] text-gray-500" />}
                        label="Teléfono" 
                        value={wsDetail.phone || "-"} 
                      />
                      <DetailRow 
                        icon={<Hash size={16} className="sm:w-[18px] sm:h-[18px] text-gray-500" />}
                  ...
                  <div>
                    ...
                    <div className="border border-gray-200 rounded-lg sm:rounded-[14px] overflow-hidden">
                      {/* Tabla desktop */}
                      <div className="hidden md:block overflow-x-auto">
                        <table className="w-full text-xs sm:text-sm">
                          <thead className="bg-gray-50 text-gray-600">
                            <tr>
                              <th className="p-2 sm:p-3 text-left font-medium">Nombre</th>
                              <th className="p-2 sm:p-3 text-left font-medium">Email</th>
                              <th className="p-2 sm:p-3 text-left font-medium">DNI</th>
                              <th className="p-2 sm:p-3 text-left font-medium">Rol</th>
                            ...
                            {members.length === 0 ? (
                              <tr>
                                <td colSpan={5} className="text-center py-8 text-gray-500">
                                  <Users size={24} className="sm:w-8 sm:h-8 mx-auto mb-2 text-gray-400" />
                                  <p className="text-xs sm:text-sm">Sin personal asignado</p>
                                </td>
                              </tr>
                            ) : (
                              members.map((m) => (
                                <tr key={m.user_id} className="border-t hover:bg-gray-50 transition-colors">
                                  ...
                                  <td className="p-2 sm:p-3 text-xs sm:text-sm text-gray-600">{m.email || "-"}</td>
                                  <td className="p-2 sm:p-3 text-xs sm:text-sm text-gray-600">{m.dni || "-"}</td>
                                  ...
                                  <td className="p-2 sm:p-3 text-center">
                                    <div className="flex items-center justify-center gap-1 sm:gap-2">
                                      <button
                                        className="inline-flex items-center gap-1 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-[4px] border border-red-300 text-red-600 hover:bg-red-50 transition-colors text-[10px] sm:text-xs font-medium"
                                      ...
                                      <button
                                        className="inline-flex items-center justify-center p-1.5 sm:p-2 rounded-[4px] border border-gray-300 text-gray-600 hover:bg-gray-100 transition-colors"
                      ...
                      <div className="md:hidden">
                        {members.length === 0 ? (
                          ...
                          <div className="divide-y divide-gray-200">
                            {members.map((m) => (
                              <div key={m.user_id} className="p-3 space-y-2">
                                <div className="flex items-center justify-between">
                                  <div className="flex-1 min-w-0">
                                  ...
                                  <span className="inline-flex items-center px-2 py-1 rounded-full text-[10px] font-medium bg-blue-100 text-blue-800 ml-2 flex-shrink-0">
                                    {formatRole(m)}
                                  </span>
                                </div>
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-gray-900">{m.dni || "-"}</div>
                                  </div>
                                </div>
                                <div className="flex items-center gap-2 pt-1">
                                  <button
                                    className="flex-1 inline-flex items-center justify-center gap-1 px-2 py-1.5 rounded-[4px] border border-red-300 text-red-600 hover:bg-red-50 transition-colors text-xs font-medium"
                                    onClick={() => unassignMember(m.user_id)}
                                    disabled={actionBusy === `kick-${m.user_id}`}
                                    title="Desvincular persona"
                                  >
            ...
            {selectedMember && (
              <div className="absolute right-0 top-0 h-full w-full sm:w-[420px] bg-white border-l shadow-xl z-[62] flex flex-col">
                <div className="flex items-center justify-between px-3 sm:px-4 md:px-5 py-2 sm:py-3 md:py-4 border-b bg-white">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    <div className="p-1.5 sm:p-2 rounded-full bg-[#F3F6FF] flex-shrink-0">
                      <User size={14} className="sm:w-[18px] sm:h-[18px] text-[#0040B8]" />
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-xs sm:text-sm font-semibold text-gray-900">Información del Usuario</h4>
                  ...
                  <button
                    ...
                    className="p-1 sm:p-1.5 rounded-full border hover:bg-gray-50 transition-colors flex-shrink-0 ml-2"
                    aria-label="Cerrar panel"
                  >
                    <X size={14} className="sm:w-4 sm:h-4 text-gray-700" />
                  </button>
                </div>

                <div className="px-3 sm:px-4 md:px-5 py-3 sm:py-4 md:py-5 space-y-3 sm:space-y-4 md:space-y-5 overflow-y-auto flex-1">
                  {/* Información Personal */}
                  <div>
                    <h5 className="text-[10px] sm:text-xs font-semibold text-gray-700 mb-2 sm:mb-3 flex items-center gap-2">
                      <User size={12} className="sm:w-3.5 sm:h-3.5 text-[#0040B8]" />
                      Información Personal
                    </h5>
                    <div className="bg-gray-50 rounded-lg sm:rounded-[14px] p-3 sm:p-4 space-y-2 sm:space-y-3 border border-gray-100">
                      <DetailRow 
                        icon={<User size={14} className="sm:w-4 sm:h-4 text-gray-500" />}
                        label="Nombre completo" 
                        value={`${selectedMember.first_name || "-"} ${selectedMember.last_name || ""}`} 
                      />
                      <DetailRow 
                        icon={<CreditCard size={14} className="sm:w-4 sm:h-4 text-gray-500" />}
                        label="DNI" 
                        value={selectedMember.dni || "-"} 
                      />
                      <DetailRow 
                        icon={<Briefcase size={14} className="sm:w-4 sm:h-4 text-gray-500" />}
                        label="Rol" 
                  ...
                  <div>
                    <h5 className="text-[10px] sm:text-xs font-semibold text-gray-700 mb-2 sm:mb-3 flex items-center gap-2">
                      <Mail size={12} className="sm:w-3.5 sm:h-3.5 text-[#0040B8]" />
                      Información de Contacto
                    </h5>
                    <div className="bg-gray-50 rounded-lg sm:rounded-[14px] p-3 sm:p-4 space-y-2 sm:space-y-3 border border-gray-100">
                      <DetailRow 
                        icon={<Mail size={14} className="sm:w-4 sm:h-4 text-gray-500" />}
                        label="Email" 
                        value={selectedMember.email || "-"} 
                      />
                      <DetailRow 
                        icon={<Phone size={14} className="sm:w-4 sm:h-4 text-gray-500" />}
                        label="Teléfono" 
                  ...
                  {(String(selectedMember.role).toLowerCase() === "ingeniero" || 
                    ...
                    <div>
                      <h5 className="text-[10px] sm:text-xs font-semibold text-gray-700 mb-2 sm:mb-3 flex items-center gap-2">
                        <Award size={12} className="sm:w-3.5 sm:h-3.5 text-[#0040B8]" />
                        Información Profesional
                      </h5>
                      <div className="bg-blue-50 rounded-lg sm:rounded-[14px] p-3 sm:p-4 space-y-2 sm:space-y-3 border border-blue-100">
                        <DetailRow 
                          icon={<Award size={14} className="sm:w-4 sm:h-4 text-blue-600" />}
                          label="Número de matrícula" 
                          value={selectedMember.license_number || "-"} 
                        />
                        <DetailRow 
                          icon={<FileText size={14} className="sm:w-4 sm:h-4 text-blue-600" />}
                          label="Título" 
                ...
                <div className="flex items-center justify-end gap-2 sm:gap-3 px-3 sm:px-4 md:px-5 py-2 sm:py-3 border-t bg-gray-50">
                  <button
                    onClick={closeMemberModal}
                    className="w-full sm:w-auto px-4 py-2 rounded-[4px] text-xs sm:text-sm font-medium bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 transition-colors"
                  >
                    Cerrar
                  </button>
                </div>
            ...
            {/* Footer */}
            <div className="flex items-center justify-end gap-2 sm:gap-3 px-3 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 border-t bg-gray-50">
              <button
                onClick={approveWorkshop}
                disabled={actionBusy !== null || loadingDetail}
                className={`w-full sm:w-auto px-4 sm:px-5 py-2 sm:py-2.5 rounded-[4px] text-xs sm:text-sm font-medium inline-flex items-center justify-center gap-2 transition-colors ${
...
function DetailRow({ icon, label, value }: { icon: React.ReactNode; label: string; value?: string }) {
  return (
    <div className="flex items-start gap-2 sm:gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[10px] sm:text-xs font-medium text-gray-500 mb-0.5">{label}</div>
        <div className="text-xs sm:text-sm text-gray-900 break-words">{value || "-"}</div>
      </div>
    </div>
  );
}
