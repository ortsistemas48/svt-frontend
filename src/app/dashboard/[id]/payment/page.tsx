...
export default function PaymentPage() {
  ...
  return (
    <div className="bg-white">
      <div className="w-full px-0 sm:px-4 md:px-6 lg:px-8 py-3 sm:py-6">
        {/* Migas */}
        <article className="mb-2 sm:mb-4 md:mb-6 flex items-center justify-between text-xs sm:text-sm md:text-base lg:text-lg px-1 sm:px-0">
        ...
        {(errorMsg || okMsg) && (
          <div className="mb-4 px-1 sm:px-0">
            {errorMsg && (
              <div className="rounded-[4px] border border-rose-300 bg-rose-50 px-3 py-2 text-xs sm:text-sm text-rose-700">
                {errorMsg}
              </div>
            )}
            {okMsg && (
              <div className="rounded-[4px] border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs sm:text-sm text-emerald-700">
                {okMsg}
              </div>
            )}
          </div>
        ...
        <section className="w-full px-1 sm:px-0">
          <div className="rounded-xl sm:rounded-2xl border border-gray-200 bg-white shadow-sm">
            {/* Encabezado compacto */}
            <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-2 sm:gap-2 border-b border-gray-100 p-3 sm:p-5">
              <span className="inline-flex items-center gap-2 rounded-full border px-2 sm:px-3 py-1 text-[10px] sm:text-xs text-gray-700">
              ...
              <span
                className={clsx(
                  "inline-flex items-center gap-2 rounded-full border px-2 sm:px-3 py-1 text-[10px] sm:text-xs",
                  stockClasses
                )}
            ...
            <div className="p-3 sm:p-4 md:p-6">
              {/* Ingreso único */}
              <div className="mx-auto max-w-3xl rounded-xl sm:rounded-2xl border border-gray-200 bg-white/60 p-3 sm:p-4 md:p-6">
                <label htmlFor="qty" className="mb-2 sm:mb-3 block text-center text-xs sm:text-sm text-gray-700">
                ...
                <div className="mt-2 flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3">
                  <input
                    ...
                    className="w-full sm:w-40 rounded-[4px] border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-[#0040B8]"
                  />
              ...
              <div className="mx-auto mt-4 sm:mt-6 max-w-3xl rounded-xl sm:rounded-2xl border border-gray-200 bg-gradient-to-b from-white to-[#F8FAFF] p-3 sm:p-4 md:p-5">
                <div className="mb-2 sm:mb-3 flex items-center gap-2">
                  <div className="rounded-[4px] bg-[#F3F6FF] p-1.5 sm:p-2 text-[#0040B8]">
                    <Info className="h-4 w-4 sm:h-5 sm:w-5" />
                  </div>
                  <h4 className="text-xs sm:text-sm font-semibold text-gray-900">Resumen</h4>
                </div>
                ...
                <button
                  onClick={openModal}
                  className="mt-4 sm:mt-5 inline-flex w-full items-center justify-center gap-2 rounded-[4px] bg-[#0040B8] px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold text-white shadow-sm hover:bg-[#00379f]"
                >
        ...
        <div className="mt-6 sm:mt-8 md:mt-10 px-1 sm:px-0">
          <div className="mb-4 sm:mb-6">
            <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-900">Pack de revisiones</h3>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-2 sm:p-4 overflow-y-auto overscroll-none">
            <div className="w-full max-w-[90vw] sm:max-w-lg md:max-w-2xl max-h-[calc(100vh-2rem)] overflow-hidden rounded-xl sm:rounded-2xl border border-gray-200 bg-white shadow-xl my-4 flex flex-col">
              {/* Header */}
              <div className="flex items-center justify-between border-b p-3 sm:p-4 flex-shrink-0">
                <h3 className="text-sm sm:text-base md:text-lg font-semibold">Transferencia bancaria</h3>
              ...
              <div className="space-y-3 sm:space-y-4 md:space-y-6 p-3 sm:p-4 md:p-5 overflow-y-auto flex-1 scrollbar-hide">
                {/* Paso 1, monto */}
                <section className="rounded-[4px] border bg-white/60 p-3 sm:p-4">
                  <h4 className="text-xs sm:text-sm font-semibold text-gray-900">1- Monto a transferir</h4>
                  <p className="mt-2 text-xl sm:text-2xl md:text-3xl font-bold tracking-tight text-[#0040B8]">{formatARS(total)}</p>
                ...
                <h4 className="text-xs sm:text-sm font-semibold text-gray-900">3- Subí el comprobante <span className="text-gray-500">(opcional)</span></h4>
                <div className="mt-2">
                ...
                {proofError && (
                    <p className="mt-2 text-xs sm:text-sm text-rose-700">{proofError}</p>
                )}
                </section>
              </div>
                ...
                <button
                    onClick={closeModal}
                    className="rounded-[4px] border px-3 sm:px-4 py-2.5 sm:py-2 text-xs sm:text-sm text-gray-700 hover:bg-gray-50 w-full sm:w-auto"
                    type="button"
                >
                    Cancelar
                </button>
...
function SummaryItem({ label, value, highlight = false }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div
      className={clsx(
        "rounded-[4px] border p-3 sm:p-4 text-center",
    ...
    >
      ...
      <div className={clsx("mt-1 text-base sm:text-lg font-semibold", highlight ? "text-[#0040B8]" : "text-gray-900")}>{value}</div>
    </div>
  );
}
...
function CopyRow({
  ...
  return (
    <li className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
      <div className="text-gray-600 break-all sm:break-normal">
        {label}: <span className="text-gray-900">{value}</span>
      </div>
      <button
        type="button"
        onClick={() => onCopy(value, k)}
        className="inline-flex items-center gap-1.5 rounded-[4px] border px-2 py-1 text-xs text-gray-700 hover:bg-gray-50 self-start sm:self-auto"
      >
